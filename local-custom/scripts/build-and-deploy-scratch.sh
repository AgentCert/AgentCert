...
ensure_litmus_exp_runtime_rbac() {
    ...
    local infra_sa=""
        ...
        if kubectl -n "$infra_namespace" get deployment subscriber &> /dev/null; then
            ...
            safe_rollout_restart "subscriber" "$infra_namespace"
        else
...
safe_rollout_restart() {
    local deployment=$1
    local ns=${2:-"$NAMESPACE"}

    if kubectl get deployment "$deployment" -n "$ns" &>/dev/null; then
        print_info "Restarting deployment: $deployment"
        kubectl rollout restart deployment/"$deployment" -n "$ns" || true
    else
        print_warning "Deployment $deployment does not exist yet in namespace $ns, skipping restart"
    fi
}
...
wait_for_deployment() {
    ...
    while [ $elapsed -lt $timeout ]; do
        if kubectl get deployment "$deployment" -n "$ns" &>/dev/null; then
            print_success "Deployment $deployment is now available"
            return 0
        fi
        print_info "Waiting for deployment $deployment to be created... ($elapsed/$timeout)s"
        sleep 2
        elapsed=$((elapsed + 2))
    done

    print_warning "Deployment $deployment did not appear within $timeout seconds"
    return 1
}
...
deploy_manifest() {
    ...
    safe_rollout_restart "litmusportal-auth-server" "$NAMESPACE"
    safe_rollout_restart "litmusportal-server" "$NAMESPACE"
...
verify_pods() {
    ...
    print_info "Waiting for deployments to be created..."
    wait_for_deployment "litmusportal-auth-server" "$NAMESPACE" 60 || print_warning "Auth server deployment not found"
    wait_for_deployment "litmusportal-server" "$NAMESPACE" 60 || print_warning "GraphQL server deployment not found"

    print_info "Waiting for rollouts to complete..."
    if kubectl get deployment "litmusportal-auth-server" -n "$NAMESPACE" &>/dev/null; then
        kubectl rollout status deployment/litmusportal-auth-server -n "$NAMESPACE" --timeout=120s || true
    fi
    if kubectl get deployment "litmusportal-server" -n "$NAMESPACE" &>/dev/null; then
        kubectl rollout status deployment/litmusportal-server -n "$NAMESPACE" --timeout=120s || true
    fi
    ...
    while [ $attempt -lt $max ]; do
        ...
        print_info "Waiting for pods ready... ($attempt/$max)"
        sleep 5
    done
...
sync_envs_if_namespace_exists() {
    if kubectl get namespace "$NAMESPACE" &> /dev/null; then
        local missing=0
        kubectl -n "$NAMESPACE" get configmap litmus-portal-admin-config &>/dev/null || missing=1
        kubectl -n "$NAMESPACE" get secret litmus-portal-admin-secret &>/dev/null || missing=1
        kubectl -n "$NAMESPACE" get deployment litmusportal-server &>/dev/null || missing=1

        if [ "$missing" -eq 1 ]; then
            print_info "First-time setup detected: cluster config resources are not created yet. Skipping pre-deploy env sync."
            return 0
        fi
...
main() {
    ...
    [ "$SKIP_DEPLOY" = false ] && { create_namespace; deploy_manifest; sync_langfuse_env_from_dotenv; sync_azure_env_from_dotenv; sync_install_agent_env_from_dotenv; ensure_litmus_exp_runtime_rbac; safe_rollout_restart "litmusportal-server" "$NAMESPACE"; verify_pods; } || print_warning "Skipping deploy"
